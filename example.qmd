...
You can also customize the summary text with the `output-summary` chunk option:
...
:::: {.callout-note}
- The `output-fold` chunk option only works with HTML output formats.
...
- Both `output-fold` and `output-summary` are chunk options (use `#|` prefix).
:::
